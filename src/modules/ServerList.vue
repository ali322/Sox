  ...
  <div class="py-2 h-full flex flex-col w-[166px] bg-gray-100">
    ...
    >
      <div
        ...
        v-for="(server, i) in sortedServers"
        :key="i"
        @click="emit('select', i)"
      >
        <div
          class="flex-1 px-2 py-2 relative flex justify-center items-center"
          :class="[index == i ? 'bg-blue-500' : 'bg-white']"
        >
          <div class="leading-4 flex-1 text-left text-sm">
            <p class="text-sm" :class="index === i && 'text-white'">
              {{ server.tag }}
            </p>
            <p
              class="text-xs"
    ...
    <div
      class="flex mx-2 justify-center border-y border-gray-300 bg-light-300 items-center"
    >
      <div
      ...
      >
        <AddFilled
          class="w-4 h-4"
          :class="running ? 'text-gray-300' : 'text-gray-500'"
        ></AddFilled>
      </div>
      <div
...
const ping = () => {
  const addrs = props.servers.map((server: Record<string, any>) => {
    if (server.data) {
      return server.data.outbounds[0].server
    } else {
      return '8.8.8.8'
    }
  })
